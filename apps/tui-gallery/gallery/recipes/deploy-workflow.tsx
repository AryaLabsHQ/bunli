...
export function DeployWorkflowRecipe({
  focusToken,
  previewWidth,
  stateKey
  ...
  const initialValues: Partial<z.infer<typeof deploymentSchema>> = stateKey === 'production'
  ...
  return (
    <Stack gap={1}>
      <Form
        ...
        onSubmit={(values) => {
          setStatus(`Prepared ${values.app} for ${values.region}${values.confirm ? ' with confirmation' : ''}.`)
        }}
        onValidationError={() => {
