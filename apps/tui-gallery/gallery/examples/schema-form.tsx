import { KeyValueList, SchemaForm, type SchemaField, Stack } from "@bunli/tui/interactive";
...
export function SchemaFormExample({ focusToken, previewWidth, stateKey }: GalleryRenderContext) {
  const [status, setStatus] = useState(
    "SchemaForm turns field metadata into a complete interactive form.",
  );
  const [lastSubmitted, setLastSubmitted] = useState<ScaffoldValues | null>(null);

  const initialValues: Partial<ScaffoldValues> =
    stateKey === "service"
      ? {
          name: "bunli-api",
          packageManager: "pnpm",
  ...
  const fields = useMemo<SchemaField<typeof scaffoldSchema>[]>(
    () => [
      ...
      },
      {
        ...
        deriveDefault: (values) =>
          Array.isArray(values.features) && values.features.includes("auth")
            ? "bunli-admin"
            : undefined,
      },
      {
      ...
      <KeyValueList
        items={[
          ...
          { key: "profile", value: stateKey === "service" ? "service starter" : "docs starter" },
          {
            key: "features",
            value:
              lastSubmitted?.features?.join(", ") ?? initialValues.features?.join(", ") ?? "none",
          },
