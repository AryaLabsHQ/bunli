...
export function FieldSetExample({ focusToken, previewWidth, stateKey }: GalleryRenderContext) {
  const [status, setStatus] = useState(
    "Move through the fields and submit to inspect the combined values.",
  );
    ...
    <Stack gap={1}>
      ...
      <Form
        ...
        onSubmit={(values) => {
          ...
          setStatus(
            `Prepared ${values.environment} run with ${values.capabilities.length} selected capabilities.`,
          );
        }}
        onValidationError={() => {
      ...
      <KeyValueList
        items={[
          ...
          {
            key: "capabilities",
            value: (lastSubmitted?.capabilities ?? baselineProfile.capabilities).join(", "),
          },
