...
export function GalleryShell({
  ...
  const activeCategory = useMemo(
    () =>
      activeSection?.categories.find((category) => category.id === categoryId) ??
  ...
  useEffect(() => {
    ...
    const category =
      activeSection.categories.find((candidate) => candidate.id === categoryId) ??
      activeSection.categories[0];
    if (!category) return;
    ...
    const entry =
      category.entries.find((candidate) => candidate.id === entryId) ?? category.entries[0];
    if (!entry) return;
    if (entry.id !== entryId) {
      setEntryId(entry.id);
    }
  ...
  useEffect(() => {
    ...
    const nextState =
      activeEntry.states.find((state) => state.key === stateKey)?.key ?? firstStateKey(activeEntry);
  ...
  useKeyboard((key) => {
    ...
    const scrollTarget =
      focusRegion === "sections" || focusRegion === "categories"
  ...
  const activeCategoryFromLookup = activeEntryRecord
    ? findCategory(gallerySections, activeEntryRecord.category.id)
    : activeCategory;
  ...
  const handleSectionChange = (nextSectionId: string) => {
    ...
    setStateKey(
      firstStateKey(nextSection.categories[0]?.entries[0] ?? activeEntry ?? fallbackEntry()),
    );
  };
  ...
  const handleCategoryChange = (nextCategoryId: string) => {
    const nextCategory = activeSection?.categories.find(
    ...
    <ThemeProvider theme={theme}>
      <Container border padding={1} style={{ height: shellHeight }}>
        <SidebarLayout
          ...
          status={
            <text
              content={`status :: ${summaryLine || "No entry selected"} :: mode=${layoutMode} :: pane=${activePane}`}
            />
          }
        />
      </Container>
