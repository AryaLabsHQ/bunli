...
export default defineCommand({
  name: 'deploy',
  options: {
    env: defineOption(z.enum(['dev', 'staging', 'production']))
  },
...
test('interactive setup', async () => {
  const cli = createTestCLI({ commands: [setupCommand] })

  // Mock prompt responses using stdin array
  const result = await cli.run(['setup'], {
    stdin: [
      'my-project',     // Project name
      'y',              // Use TypeScript
      'tests,git',      // Features (comma-separated)
      'y'               // Confirm
    ]
  })
...
```typescript
import { test, expect } from 'bun:test'
import { createTestCLI, mockPromptResponses } from '@bunli/test'

test('interactive setup with mockPromptResponses', async () => {
  const cli = createTestCLI({ commands: [setupCommand] })

  // Map prompt messages to responses
  const result = await cli.run(['setup'], {
    stdin: mockPromptResponses({
