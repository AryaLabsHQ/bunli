...
Bunli prompts are implemented on top of `@clack/prompts`. For advanced primitives (like `intro/outro`), use `prompt.clack`:

```typescript
export default defineCommand({
  handler: async ({ prompt }) => {
    prompt.clack.intro('Setup')
    const name = await prompt.clack.text({ message: 'Name' })
    if (prompt.clack.isCancel(name)) {
