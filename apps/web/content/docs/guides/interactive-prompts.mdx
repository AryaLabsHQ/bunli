...
Filter through a list of items interactively:

```typescript
export default defineCommand({
  name: "search",
  handler: async ({ prompt }) => {
    const results = await prompt.filter("Select items to install:", {
      options: [
        { value: "auth", label: "Authentication" },
        { value: "db", label: "Database" },
        { value: "email", label: "Email Service" },
    ...
    console.log(`Selected: ${results.join(", ")}`);
  },
});
```
...
export default defineCommand({
  ...
  handler: async ({ prompt }) => {
    await prompt.pager(`
      ...
      [2024-01-15 10:23:48] WARN: Deprecated API endpoint called
      [2024-01-15 10:23:49] ERROR: Failed to send email
    `);
  },
});
```
...
<Callout type="tip">
  Generated types work perfectly with interactive commands, providing type safety for dynamic prompt
  generation and option validation.
</Callout>
...
- [TUI Gallery](/docs/guides/tui-gallery) - Runnable component examples and runtime recipes for `@bunli/tui`
