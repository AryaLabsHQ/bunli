...
Bunli prompt primitives are provided by `@bunli/runtime`, and the handler `prompt` utility exposes lifecycle helpers like `intro/outro/note/cancel`:
...
```typescript
// clack mental model -> @bunli/runtime
...
- [@bunli/tui](/docs/packages/tui) - TUI components and render buffer-mode reference
