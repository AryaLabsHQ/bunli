...
const emailOption = defineOption(
  v.string([v.email()])
)
```
...
export default defineCommand({
  options: {
    ...
    // Email validation
    email: defineOption(z.string().email('Invalid email address')),
    
    // URL validation
    webhook: defineOption(z.string().url('Invalid URL')),
...
export default defineCommand({
  options: {
    ...
    // Optional boolean
    debug: defineOption(z.coerce.boolean().optional()),
    
    // Boolean with default
    production: defineOption(z.coerce.boolean().default(false))
  }
})
...
export default defineCommand({
  options: {
    ...
    // Date range
    startDate: defineOption(z.coerce.date()),
    endDate: defineOption(z.coerce.date())
  },
  handler: async ({ flags }) => {
    // Validate date range
...
export default defineCommand({
  options: {
    deploy: defineOption(deploySchema)
  }
})
```
...
export default defineCommand({
  name: 'api-call',
  options: {
    config: defineOption(apiConfigSchema)
  }
})
```
