...
```typescript
interface RunnableCommand<TOptions = Options, TStore = {}> {
  name: string
  description: string
  alias?: string | string[]
  options?: TOptions
...
const command = defineCommand({
  ...
  options: {
    ...
    host: defineOption(z.string().default('localhost'))
  },
  handler: async ({ flags }) => {
    // TypeScript knows:
...
export default defineCommand({
  ...
  options: {
    name: defineOption(z.string().default('World'), { short: 'n' }),
    loud: defineOption(z.coerce.boolean().default(false), { short: 'l' })
  },
  render: ({ flags }) => (
    <GreetProgress
