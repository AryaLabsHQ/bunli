...
- `renderer.exitOnCtrlC` - Exit TUI on Ctrl+C
- `renderer.targetFps` - Target frame rate for TUI rendering
- `renderer.enableMouseMovement` - Enable mouse movement tracking
- `renderer.useMouse` - Enable mouse support
...
<Callout type="tip">
  **Automatic Loading**: In most cases, you don't need to manually load config. `createCLI()`
