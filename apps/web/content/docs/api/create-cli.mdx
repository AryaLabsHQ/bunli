...
Override specific configuration values or provide configuration inline. The `generated` option defaults to `true` — set to `false` to disable type generation.

**Note**: When providing a `plugins` array with typed plugins, pass it as a `const` tuple to preserve type information:
```typescript
...
interface CLI<TStore = any> {
  ...
  execute(commandName: string, argsOrOptions?: string[] | Record<string, unknown>, options?: Record<string, unknown>): Promise<void>
}
```
