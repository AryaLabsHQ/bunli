...
<Callout type="tip">
  **New in Bunli 0.4.0**: Configuration is automatically loaded from `bunli.config.ts`. You only
  need to provide overrides for runtime-specific settings like plugins with custom options.
</Callout>
...
// Execute with args only
await cli.execute("my-command", ["--flag", "value"]);

// Execute with options
await cli.execute("deploy", { env: "production" });
```
...
<Callout type="tip">
  The CLI automatically handles help flags at any level. For example, `my-cli build --help` shows
  help for the build command specifically.
</Callout>
