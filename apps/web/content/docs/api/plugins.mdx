...
Per-run state shared between `preRun` and `postRun` hooks across all plugins.
...
Plugins use `ExecutionState` to pass data from `preRun` to `postRun` without coupling:
...
postRun?(context, state) {
  const elapsed = Date.now() - state.get('startTime')
