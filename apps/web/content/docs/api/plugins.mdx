...
Create a plugin for testing and development. Useful for mocking plugin behavior in tests.
...
Compose multiple plugins into a single plugin with merged store.
...
Extract the options type from a plugin factory.

```typescript
type InferPluginOptions<T> = T extends PluginFactory<infer O, any> ? O : never;

// Example
type Options = InferPluginOptions<typeof myPluginFactory>;
```

### InferPluginStore\<T\>

Extract the store type from a plugin or plugin factory.
...
class ExecutionState {
  ...
  set<K extends string>(value: unknown): void;
