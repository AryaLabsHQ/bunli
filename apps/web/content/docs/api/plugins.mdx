...
Combine multiple plugins into a single plugin with merged store types.

```typescript
function composePlugins<P1 extends BunliPlugin, P2 extends BunliPlugin>(
  plugin1: P1,
  plugin2: P2
): BunliPlugin<MergeStores<[P1, P2]>>

// Compose multiple plugins
const combined = composePlugins(pluginA, composePlugins(pluginB, pluginC))
```

### createTestPlugin

Create a plugin for testing purposes with optional mocked behavior.

```typescript
function createTestPlugin<TStore = {}>(
  plugin?: Partial<BunliPlugin<TStore>>
): BunliPlugin<TStore>
```

### ExecutionState

A key-value store for sharing data between plugins during a single command execution. Available in `preRun` and `postRun` hooks.
...
// Example: sharing data between preRun and postRun
const timingPlugin = {
  name: 'timing',
