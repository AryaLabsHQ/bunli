...
function definePlugin<TStore = {}>(
  plugin: BunliPlugin<TStore>
): BunliPlugin<TStore>

// Factory overload
function definePlugin<TOptions, TStore = {}>(
...
const myPlugin = definePlugin<Options, MyStore>((options) => ({
  name: 'my-plugin',
  store: { count: 0 },
...
```typescript
import { definePlugin, type BunliPlugin } from '@bunli/core/plugin'
...
export const metricsPlugin = definePlugin<{ detailed?: boolean }, MetricsStore>(
  (options = {}) => ({
