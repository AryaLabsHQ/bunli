...
### isCommandFile Function

Check whether a file should be treated as a command file (skips test files, node_modules, etc.):

```typescript
import { isCommandFile } from '@bunli/generator'

isCommandFile('./src/commands/deploy.ts') // true
isCommandFile('./src/commands/deploy.test.ts') // false
isCommandFile('./node_modules/foo/index.ts') // false
```
...
```typescript
import { Result } from '@bunli/core'

class Generator {
  constructor(config: GeneratorConfig)

  // Generate types
  run(event?: GeneratorEvent): Promise<Result<void, GeneratorRunError>>
