...
interface StoreInstance<TConfig> {
  ...
  update(updater: (current: TConfig) => TConfig): Promise<void>
...
Extract the field type from a `StoreOptions` definition for use in function signatures:

```typescript
import { createStore, type InferStoreConfig } from '@bunli/store'

type MyConfig = InferStoreConfig<{
  theme: { type: 'string'; default: 'dark' }
  port: { type: 'number'; default: 3000 }
}>
// → { theme: string; port: number }

function useStore(config: InferStoreConfig<typeof defaultOptions>) {
  const store = createStore(config)
  // store.get() returns fully typed values
}
```
