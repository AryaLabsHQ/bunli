...
The `@bunli/cli` npm package exports configuration helpers and utilities for use in `bunli.config.ts`:

```typescript
import { defineConfig, loadConfig } from "@bunli/cli";
import { findEntry } from "@bunli/cli";
```

The `bunli` CLI binary (installed via `bun add -g bunli`) provides the commands listed below (`bunli dev`, `bunli build`, etc.). These are entry points executed via the binary, not imports from the package.
...
<Callout type="info">
  Type generation is automatically run during `bunli dev` and `bunli build`. You typically only need
...
<Callout type="warning">
  Use `--npm=false` (not `--no-npm`) to disable npm publishing. Boolean options work as `--flag`
...
<Callout type="info">
  `bunli build` can optionally compress multi-target builds into `dist/&lt;target&gt;.tar.gz`. For
