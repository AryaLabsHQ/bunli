...
<Callout type="info">
  Prompt primitives now live in `@bunli/runtime`.
</Callout>
...
Prompt APIs are provided by `@bunli/runtime`:
...
Progress indicators are provided via handler-injected `spinner` (backed by `@bunli/runtime`).
