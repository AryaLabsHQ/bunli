...
Re-exported from `@standard-schema/utils`. Retrieves a nested value from an object using a dot-notation path:
...
getDotPath(obj, 'user.profile.name')  // 'Alice'
getDotPath(obj, 'user.profile.age')   // undefined

// Useful for extracting config values from nested objects
const config = { build: { outdir: 'dist' } }
const outdir = getDotPath(config, 'build.outdir') ?? 'build'
```
