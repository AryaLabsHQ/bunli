...
Shared utilities for building CLI applications. Includes colors and validation helpers.

<Callout type="info">
  Prompt primitives now live in `@bunli/runtime/prompt`.
</Callout>
...
Prompt APIs are provided by `@bunli/runtime/prompt`:
...
Progress indicators are provided via handler-injected `spinner` (backed by `@bunli/runtime/prompt`).
...
When working with generated types from `@bunli/generator`, you can use advanced type utilities from `@bunli/core` for complex type manipulation:
...
<Callout type="info">
  Type utilities are exported from `@bunli/core`, not `@bunli/utils`. They're designed to work with generated types for advanced type manipulation patterns.
</Callout>
