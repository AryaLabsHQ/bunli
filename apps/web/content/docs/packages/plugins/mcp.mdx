...
Beyond the main plugin, `@bunli/plugin-mcp` exports utilities for custom MCP integrations:

### Metadata Extraction

```typescript
import { extractCommandMetadata, extractSchemaMetadata } from '@bunli/plugin-mcp'
import type { MCPCommandMetadata, SchemaMetadata, SchemaConversionOptions } from '@bunli/plugin-mcp'

// Extract command metadata from an MCP tool
const meta = extractCommandMetadata(mcpTool)
// → { name, description, options: { ... }, filePath, ... }

// Extract detailed schema metadata for completions
const schemaMeta = extractSchemaMetadata(jsonSchema)
// → { type, required, enumValues, minLength, maxLength, ... }
```

### Type Code Generation

```typescript
import { generateMCPTypes } from '@bunli/plugin-mcp'

// Generate TypeScript type definitions from MCP tools
const result = await generateMCPTypes({
  tools: [{ namespace: 'linear', tools: linearTools }],
  outputDir: '.bunli'
})
// Writes .bunli/mcp-linear.gen.ts with Zod schemas and type definitions
```
...
```typescript
import {
  toKebabCase, toCommandName, toFlagName,
  toPascalCase, toCamelCase, escapeString
} from '@bunli/plugin-mcp'

toCommandName('list-issues')    // 'list-issues'
toFlagName('list_issues')       // 'list-issues'
toKebabCase('listIssues')        // 'list-issues'
escapeString("it's a \"test\"")  // "it\\'s a \\"test\\"" (safe for TS literals)
```
