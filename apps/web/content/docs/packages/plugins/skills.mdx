...
import type { Agent } from '@bunli/plugin-skills'

// All known agent definitions (Agent[] — not strings)
builtinAgents
// → [{ name: 'Claude Code', globalSkillsDir: '~/.claude/skills', ... }, ...] (30+ agents)

// Detect which agents are installed on the system (synchronous)
const agents = detectAgents()
console.log(agents)
// → [{ name: 'Claude Code', globalSkillsDir: '~/.claude/skills', projectSkillsDir: '.claude/skills', universal: false, detect: [Function] }, ...]
```
...
// Generate a full SKILL.md string for all commands (synchronous)
const skillFile = generateSkillFile(
  'my-cli',          // CLI name
  commandsMap,       // Map<string, Command>
  { description: 'My CLI tool' }
)

// Generate a SKILL.md string for a single command (synchronous)
const skill = generateCommandSkill(
  'my-cli',          // CLI name
  'deploy',          // command name
...
const result = await syncSkills(
  'my-cli',          // CLI name
  commandsMap,       // Map<string, Command>
  { global: false }  // SyncOptions — install project-local instead of global
)

result.paths       // Canonical install paths (string[])
result.agents      // Per-agent install details (AgentInstall[])
result.updated     // Whether the skill file was written (false if hash matched)
```
