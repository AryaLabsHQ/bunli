...
<Callout type="tip">
  Testing generated types ensures your CLI's type generation is working correctly and provides
  confidence in type-safe CLI wrappers.
</Callout>
...
| `stdin`/`mockPrompts`      | ✅ Supported    | ❌ Not supported                         |
| `cli.run(argv)`            | —               | Run with argv like CLI invocation        |
| `cli.execute(name, args?)` | —               | Execute a named command programmatically |
