...
```typescript
// Parse JSON input
const configOption = defineOption(
...
const myPlugin = definePlugin<{ requestCount: number }>({
  name: 'my-plugin',
  store: {
    requestCount: 0
  },
  beforeCommand(context) {
    const requestCount = context.getStoreValue('requestCount')
    context.setStoreValue('requestCount', requestCount + 1)
  },
  afterCommand(context) {
    console.log(`Total requests: ${context.getStoreValue('requestCount')}`)
  }
})
...
cli.command(defineCommand({
  ...
  handler: async ({ context }) => {
    console.log(`Requests: ${context?.getStoreValue('requestCount') ?? 0}`)
  }
}))
