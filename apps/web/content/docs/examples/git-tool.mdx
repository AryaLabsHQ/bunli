...
```bash
bun run cli.ts branch create --name feature/new-feature
bun run cli.ts branch switch main
bun run cli.ts branch list
bun run cli.ts branch delete --name feature/old-feature
```
...
export const branchCommand = defineCommand({
  ...
  commands: [
    ...
    defineCommand({
      ...
      handler: async ({ positional, shell, colors }) => {
        const branchName = positional[0]
        await shell`git checkout ${branchName}`
...
// Simple command execution — tagged template literal returns stdout as string
const statusOutput = await shell`git status`
console.log(statusOutput)

// Command with options
const logOutput = await shell`git log --oneline -10`
