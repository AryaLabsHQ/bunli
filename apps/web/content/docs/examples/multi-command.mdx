...
## Generated Types for Command Structure
...
//   { name: 'complete', description: 'Mark tasks as completed' },
//   { name: 'project', description: 'Manage projects' }
// ]

// Type-safe access to nested commands
const projectApi = getCommandApi('project')
console.log(projectApi.options) // Nested command options

// Access specific nested command
const createApi = getCommandApi('project.create')
console.log(createApi.options) // { name: {...}, description: {...}, color: {...} }
```
...
<Callout type="tip">
  Generated types work seamlessly with nested command structures, providing full type safety for complex CLI hierarchies.
</Callout>
...
8. **Type Generation**: Full TypeScript definitions for nested commands
