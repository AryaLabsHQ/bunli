...
- `--targets, -t` - Target platforms for compilation (e.g., darwin-arm64,linux-x64)
...
- Traditional JavaScript builds (when no targets specified)
...
For multiple platforms, this creates:
```
dist/
  darwin-arm64/
    cli          # macOS ARM64 executable
  darwin-x64/
    cli          # macOS x64 executable
  linux-x64/
    cli          # Linux executable
  windows-x64/
    cli.exe      # Windows executable
```

With compression enabled in config, it also creates:
```
dist/
  darwin-arm64.tar.gz
  darwin-x64.tar.gz
