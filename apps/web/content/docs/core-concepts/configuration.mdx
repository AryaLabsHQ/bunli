...
export default defineConfig({
  build: {
    ...
    // Output filename (for single executable)
    outfile: 'my-cli',
    ...
    // Enable bytecode compilation (default: false)
    bytecode: false,
...
export default defineConfig({
  dev: {
    ...
    // Enable type generation (default: true)
    generate: true,

    // Clear screen on reload (default: true)
    clearScreen: true
  }
})
```
...
export default defineConfig({
  help: {
    renderer: 'default'  // Help renderer type
  }
})
```
...
export default defineConfig({
  release: {
    ...
    // Resume from unfinished release state (default: true)
    resume: true,
