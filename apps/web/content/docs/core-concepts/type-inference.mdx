...
1. **`as const` strongly recommended** - Command names should use `as const` for optimal type inference (without it, type inference still works but with less precision)
...
The generated store provides helper methods for command discovery. Import `createGeneratedHelpers` from the auto-generated `.bunli/commands.gen.ts`:
...
const cli = createGeneratedHelpers(commands, metadata);
