...
<Callout type="note">
  **Handler `context` vs Plugin `context`**: The `context` available in command handlers is `CommandContext`, which provides access to the merged plugin store and command execution data. The `PluginContext` (with `updateConfig`, `registerCommand`, `logger`, `paths`) is only available during plugin `setup` and `configResolved` hooks, not in command handlers.
</Callout>
...
import { createGeneratedHelpers } from "./.bunli/commands.gen";
import * as commands from "./commands.gen.js";
import * as metadata from "./.bunli/commands.meta.js";

// Create the generated store
const generatedCLI = createGeneratedHelpers(commands, metadata);

// Access generated CLI API
const commands = generatedCLI.list();
const deploy = generatedCLI.findByName("deploy");
...
<Callout type="tip">
  Generated types work seamlessly with plugins, providing type safety for plugin store access and
  context usage.
</Callout>
