...
Error handling behavior varies by hook. Hooks return `Result` — the framework acts on errors:

| Hook | On Error | Recommendation |
|------|----------|---------------|
| `setup` | Stops CLI init | Validate inputs; don't return errors for recoverable issues |
| `beforeCommand` | Stops command execution | Return `Err()` for validation failures; return `Ok()` to continue |
| `preRun` | Stops handler | Use for command-level guards only |
...
beforeCommand(context) {
  // Return Err() to stop command execution
  if (!isValid(context)) {
    return Err({ message: 'Validation failed', code: 'INVALID_CONTEXT' })
  }
  return Ok()
}

configResolved(config) {
  // Errors are caught internally and logged — safe for telemetry:
