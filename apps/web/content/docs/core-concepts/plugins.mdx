...
const myPlugin = definePlugin<MyPluginOptions, MyPluginStore>((options) => ({
  name: 'my-plugin',
...
const timingPlugin = definePlugin<TimingStore>({
  name: 'timing',
...
```typescript
// ✅ Good - Explicit types with definePlugin generics
interface MyStore {
  items: string[]
  count: number
}
...
// For direct plugins:
const plugin = definePlugin<MyStore>({
  name: 'my-plugin',
  store: { items: [], count: 0 }
})

// For plugin factories:
const plugin = definePlugin<Options, MyStore>((options) => ({
  name: 'my-plugin',
  store: { items: [], count: 0 }
}))
...
Error handling behavior varies by hook:

| Hook | On Error | Recommendation |
|------|----------|---------------|
| `setup` | Throws, stops CLI init | Validate inputs; don't throw for recoverable errors |
| `beforeCommand` | Throws, stops command execution | Return early on validation failure |
...
```typescript
beforeCommand(context) {
  // Errors thrown here stop command execution
  if (!isValid(context)) {
    throw new PluginHookError('Validation failed')
  }
}

configResolved(config) {
  // Errors here are logged but don't stop execution
  // Good for telemetry:
...
const myPluginFactory = definePlugin<{ verbose?: boolean }, {}>((options) => ({
  name: 'my-plugin',
...
const analyticsPlugin = definePlugin<AnalyticsStore>({
  name: 'analytics',
...
const envPlugin = definePlugin<EnvStore>({
  name: 'env-plugin',
