...
Commands can specify a default output format that applies when `--format` is not explicitly set:

```typescript
export default defineCommand({
  name: "status",
  description: "Show status information",
  defaultFormat: "toon", // 'json' | 'yaml' | 'toon' | 'md'
  handler: async ({ output }) => {
    output({ status: "ok", uptime: process.uptime() });
  },
});
```
...
- **`'yaml'`** — YAML output
- **`'toon'`** — Human-readable toon format
- **`'md'`** — Markdown output
