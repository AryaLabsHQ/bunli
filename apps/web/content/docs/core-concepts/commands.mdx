...
<Callout type="important">
  **All options must have a schema** - there are no raw options in Bunli. This ensures type safety
  and validation. Use `z.coerce` for CLI inputs to handle string-to-type conversions automatically.
</Callout>
...
<Callout type="info">
  Commands with `render` run directly without TUI flags. Buffer mode defaults to `standard`; set
  `tui.renderer.bufferMode: 'alternate'` (globally or per-command) for fullscreen UI.
</Callout>
  ...
  <Tab value="single" label="Single File">
    ...
    export default defineCommand({
      name: 'build' as const,
	    ...
	    const cli = await createCLI({ name: 'my-cli', version: '1.0.0', description: 'My CLI' })
	    cli.command(serve)
	    cli.command(build)
	    await cli.run()
	    ```
	  ...
	  <Tab value="groups" label="Command Groups">
	    For nested commands, compose explicit groups:
...
`defineGroup` creates a non-runnable command container that groups subcommands together. Unlike `defineCommand`, a group has no `handler` or `render` — it only exists to organize related commands under a shared namespace:
...
**When to use `defineGroup`:**
- Organizing related commands (e.g., `my-cli db migrate`, `my-cli db seed`)
- Creating command namespaces without needing a parent handler
- Grouping commands that share a plugin store

**Key differences from `defineCommand`:**
- `defineGroup` has no `handler` or `render` — it's purely structural
- `defineGroup` can have an `outputPolicy` and `defaultFormat` (inherited by subcommands unless overridden)
- Subcommands in a group don't inherit the parent's plugin store — they use the same store type when passed to `createCLI`
...
Commands can specify a default output format that overrides the global default:

```typescript
export default defineCommand({
  name: "status",
  description: "Show system status",
  defaultFormat: "json", // 'json' | 'yaml' | 'toon' | 'md'
  handler: async ({ output, flags }) => {
    output(getStatusData());
  },
});
```
...
- **`'json'`** — JSON output
- **`'yaml'`** — YAML output
- **`'toon'`** — Human-readable table format
- **`'md'`** — Markdown format

When `defaultFormat` is set on a command, it overrides the global `format` setting for that command unless the user explicitly passes `--format` on the CLI.
...
<Callout type="tip">
  Keep command registration explicit with `cli.command(...)` and use `defineGroup(...)` for nested
  command trees.
</Callout>
