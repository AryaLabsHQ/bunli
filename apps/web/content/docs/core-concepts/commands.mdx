...
Add typed options to your commands using the `defineOption()` helper with Zod schemas:
...
export default defineCommand({
  ...
  outputPolicy: 'always', // 'always' | 'never' | 'auto' (default)
...
- **`'auto'`** — Only show output when not a TTY (default)
- **`'always'`** — Always show structured output
- **`'never'`** — Suppress structured output entirely
