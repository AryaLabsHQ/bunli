...
export default defineCommand({
  ...
  outputPolicy: 'all', // 'all' | 'agent-only' (default: 'all')
...
- **`'all'`** — Show structured output in all environments (default)
- **`'agent-only'`** — Only show structured output when a TUI is not available (agent/headless mode)
