...
const logsCommand = defineCommand({
  ...
  options: {
    ...
    lines: option(z.coerce.number().min(1).max(1000).default(50), {
  ...
  handler: async ({ flags, spinner, colors, context }) => {
    ...
    if (follow) {
      ...
      const interval = setInterval(() => {
        ...
        const serviceName =
          service ?? services[Math.floor(Math.random() * services.length)] ?? "server";
        ...
        console.log(
          `${colors.dim(timestamp)} ${levelColor(`[${logLevel.toUpperCase()}]`)} ${colors.cyan(`[${serviceName}]`)} ${message}`,
        );
      ...
      const filteredLogs = logEntries
        .filter(
          (log) => (!service || log.service === service) && shouldIncludeLevel(log.level, level),
        )
        .slice(-lines);
      ...
      filteredLogs.forEach((log) => {
        ...
        console.log(
          `${colors.dim(log.timestamp)} ${levelColor?.(`[${log.level.toUpperCase()}]`) || colors.gray(`[${log.level.toUpperCase()}]`)} ${colors.cyan(`[${log.service}]`)} ${log.message}`,
        );
      });
    }
