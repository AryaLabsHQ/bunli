...
describe("@bunli/runtime prompt adapters", () => {
  ...
  test("password reveal key helper only matches Ctrl+R", () => {
    expect(__promptInternalsForTests.isPasswordRevealToggleKey({ ctrl: true, name: "r" })).toBe(
      true,
    );
    expect(__promptInternalsForTests.isPasswordRevealToggleKey({ name: "r" })).toBe(false);
    expect(__promptInternalsForTests.isPasswordRevealToggleKey({ ctrl: true, name: "c" })).toBe(
      false,
    );
  });
  ...
  test("intro/outro formatting helpers produce section-style labels", () => {
    ...
    const outro = stripAnsi(
      __promptInternalsForTests.formatOutroLine("Project created successfully"),
    );
  ...
  test("symbol mode resolver honors explicit env override and non-tty fallback", () => {
    expect(__promptInternalsForTests.resolvePromptSymbolMode({ BUNLI_TUI_SYMBOLS: "ascii" })).toBe(
      "ascii",
    );
    expect(__promptInternalsForTests.resolvePromptSymbolMode({ BUNLI_SYMBOLS: "unicode" })).toBe(
      "unicode",
    );
  });
  ...
  test("simulatePasswordKeySequence accepts pasted multi-character input", () => {
    const result = __promptInternalsForTests.simulatePasswordKeySequence({
      keys: [{ sequence: "super-secret-token", name: undefined }, { name: "enter" }],
    });
