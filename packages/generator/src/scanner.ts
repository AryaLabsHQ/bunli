...
export class CommandScanner {
  ...
  async scanCommands(
    entry: string,
    directory?: string,
  ): Promise<Result<string[], ScanCommandsError>> {
    const resolvedEntry = resolve(entry);
    if (!existsSync(resolvedEntry)) {
      return Result.err(
        new ScanCommandsError({
          entry: resolvedEntry,
  ...
  private async inspectModule(filePath: string): Promise<
    Result<
      {
      ...
      try: async () => {
        ...
        traverse(ast, {
          ...
          VariableDeclarator: (path: any) => {
            ...
            if (init.type === "CallExpression" && this.isCommandFactoryCall(init)) {
    ...
    return Result.tryPromise({
      try: async () => {
        ...
        const hasCommandExport = scanResult.exports.some(
          (exp) => exp === "default" || exp.includes("Command") || exp.includes("Group"),
        );
        const hasCoreImport = scanResult.imports.some((imp) => imp.path.includes("@bunli/core"));
        const hasCommandCall =
          content.includes("defineCommand(") || content.includes("defineGroup(");
