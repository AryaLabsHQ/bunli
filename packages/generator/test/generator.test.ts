...
describe("Generator", () => {
  ...
  async function writeEntryWithCommands(commandImports: string[]) {
    const importLines = commandImports
      .map((file, index) => `import command${index} from '${file}'`)
      .join("\n");
    const registerLines = commandImports
      .map((_, index) => `cli.command(command${index})`)
      .join("\n");
    const content = `
...
`,
  ...
  test("scanner ignores non-code relative imports while traversing entry graph", async () => {
    ...
    await Bun.write(
      join(testDir, "json-import-command.ts"),
      `
import { defineCommand } from '@bunli/core'
...
`,
  ...
  test("scanner ignores inline entry registrations as parse targets", async () => {
    ...
    await Bun.write(
      join(testDir, "imported-command.ts"),
      `
import { defineCommand } from '@bunli/core'
...
`,
  ...
  test("parser extracts shared option metadata from imported identifiers and spreads in nested commands", async () => {
    ...
    await Bun.write(
      join(testDir, "config/options.ts"),
      `
