...
describe("@bunli/tui dialog manager integration", () => {
  ...
  test("confirm resolves on Enter", async () => {
    ...
    const ready = await waitFor(rendered.renderOnce, () =>
      rendered.captureCharFrame().includes("Confirm deploy"),
    );
  ...
  test("choose rejects with DialogDismissedError on Escape", async () => {
    ...
    const rendered = await setup(<Harness />);
    const ready = await waitFor(rendered.renderOnce, () =>
      rendered.captureCharFrame().includes("Environment"),
    );
    expect(ready).toBe(true);
    ...
    const settled = await waitFor(
      rendered.renderOnce,
      () => rejectedError !== null || selectedValue !== null,
    );
