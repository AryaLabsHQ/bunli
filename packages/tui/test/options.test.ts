...
describe("@bunli/tui renderer options", () => {
  ...
  test("bufferMode has precedence over legacy useAlternateScreen", () => {
    ...
    expect(getUseAlternateScreen({ bufferMode: "alternate", useAlternateScreen: false })).toBe(
      true,
    );
  });
  ...
  test("resolveOpenTuiRendererOptions resolves useAlternateScreen from bufferMode", () => {
    expect(
      resolveOpenTuiRendererOptions({ bufferMode: "standard", useAlternateScreen: true })
        .useAlternateScreen,
    ).toBe(false);
    expect(
      resolveOpenTuiRendererOptions({ bufferMode: "alternate", useAlternateScreen: false })
        .useAlternateScreen,
    ).toBe(true);
  });
});
