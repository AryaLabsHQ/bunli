...
export function SidebarLayout({
  ...
  const [internalActivePane, setInternalActivePane] =
    useState<SidebarLayoutPane>(defaultActivePane);
  ...
  const selectorRow = (
    panes: SidebarLayoutPane[],
    selectedPane: SidebarLayoutPane = currentActivePane,
  ) => (
    <box
      paddingLeft={1}
      paddingRight={1}
  ...
  const renderNarrowSelector = resolvedMode === "narrow" ? selectorRow(paneOrder) : null;
    ...
    if (resolvedMode === "wide") {
      return (
        <box style={{ flexDirection: "row", gap, flexGrow: 1, height: "100%" }}>
          ...
          {inspector ? (
            <box width={inspectorWidth} height="100%">
              {inspector}
            </box>
          ) : null}
        </box>
        ...
        <box style={{ flexDirection: "row", gap, flexGrow: 1, height: "100%" }}>
          ...
          <box style={{ flexDirection: "column", gap: 1, flexGrow: 1 }} height="100%">
            {inspector
              ? selectorRow(
                  ["content", "inspector"],
                  currentActivePane === "inspector" ? "inspector" : "content",
                )
              : null}
            <box style={{ flexGrow: 1 }} height="100%">
              {sidePane}
  ...
  return (
    <box height={height} style={{ flexDirection: "column", gap: 1 }}>
      {header}
      <box style={{ flexDirection: "column", gap: 1, flexGrow: 1 }}>{mainBody}</box>
      {status}
    </box>
  );
}
