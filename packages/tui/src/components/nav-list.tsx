...
export function NavList({
  ...
  const [internalValue, setInternalValue] = useState(
    () => defaultValue ?? findFirstSelectableKey(items),
  );
  ...
  const content = (
    <>
      {title ? (
        <text
          content={formatFixedWidth(title, lineWidth, { overflow: "clip" })}
          fg={tokens.textMuted}
        />
      ) : null}
      {items.map((item) => {
        ...
        const sectionLabel =
          item.section && item.section !== previousSection ? item.section : undefined;
          ...
          <box key={item.key} style={{ flexDirection: "column", gap: compact ? 0 : 1 }}>
            ...
            >
              ...
              {!compact && item.description ? (
                <text
                  content={formatFixedWidth(item.description, Math.max(14, lineWidth - 2), {
                    overflow,
                  })}
                  fg={tokens.textMuted}
                />
  ...
  if (!boxed) {
    return (
      <box id={id} width="100%" style={{ flexDirection: "column", gap: compact ? 0 : 1 }}>
        {content}
      </box>
    );
  }
