...
export function DataTable({
  ...
  const rawRowLines = sortedRows.map((row) =>
    columns
      .map((column, index) =>
        formatFixedWidth(String(row[column.key] ?? ""), widths[index] ?? 1, { overflow }),
      )
      .join(" | "),
  );
  ...
  const footer = formatFixedWidth("Arrows: navigate/sort | Enter: select row", finalLineWidth, {
    overflow,
  });
  ...
  return (
    ...
    >
      ...
      {sortedRows.length === 0 ? (
        <text
          content={formatFixedWidth("No rows", finalLineWidth, { overflow })}
          fg={tokens.textMuted}
        />
      ) : (
        sortedRows.map((row, rowIndex) => {
          ...
          return (
            <text
              ...
              content={formatFixedWidth(`${active ? ">" : " "} ${line}`, finalLineWidth + 2, {
                overflow,
              })}
