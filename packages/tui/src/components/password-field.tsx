...
export function PasswordField({
  ...
  const display = useMemo(() => {
    const base = value.length > 0 ? visibleValue : (placeholder ?? "");
  ...
  return (
    <box style={{ flexDirection: "column", marginBottom: 1, gap: 1 }}>
      ...
      >
        <box
          style={{ backgroundColor: field.focused ? tokens.backgroundMuted : tokens.background }}
        >
          <text content={display} fg={value.length === 0 ? tokens.textMuted : tokens.textPrimary} />
        </box>
      </box>
      <text
        content={`Value: ${mask(value)} ${revealed ? "(revealed)" : "(hidden, Ctrl+R reveal)"}`}
        fg={tokens.textMuted}
      />
