...
type ReadConfigError = ConfigFileNotFoundError | ConfigFileReadError | ConfigFileParseError;
      ...
      for (const source of sources) {
        ...
        if (Result.isError(loadedConfig)) {
          ...
          if (ConfigFileParseError.is(loadedConfig.error)) {
            context.logger.warn(
              `Failed to parse config file ${configPath}: ${loadedConfig.error.message}`,
            );
            continue;
          }

          context.logger.warn(
            `Failed to load config file ${configPath}: ${loadedConfig.error.message}`,
          );
          continue;
        }
...
async function readConfigSource(
  path: string,
): Promise<Result<Record<string, unknown>, ReadConfigError>> {
