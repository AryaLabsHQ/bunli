...
test("syncSkills can sandbox global installs with an injected runtime home", async () => {
  ...
  expect(
    fs.existsSync(path.join(agentRoot, ".custom-agent", "skills", "demo-cli", "SKILL.md")),
  ).toBe(true);
});
...
test("syncSkills reruns when the detected agent set changes", async () => {
  ...
  expect(
    fs.existsSync(path.join(agentRoot, ".second-agent", "skills", "demo-cli", "SKILL.md")),
  ).toBe(true);
});
