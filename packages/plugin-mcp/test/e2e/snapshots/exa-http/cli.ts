...
const ExaWebSearchExaCommand = defineCommand({
  ...
  options: {
    'query': defineOption(z.string(), { description: 'Websearch query' }),
    'num-results': defineOption(z.coerce.number().optional(), { description: 'Number of search results to return (default: 8)' }),
    'livecrawl': defineOption(z.enum(['fallback', 'preferred']).optional(), { description: 'Live crawl mode - \'fallback\': use live crawling as backup if cached content unavailable, \'preferred\': prioritize live crawling (default: \'fallback\')' }),
    'type': defineOption(z.enum(['auto', 'fast', 'deep']).optional(), { description: 'Search type - \'auto\': balanced search (default), \'fast\': quick results, \'deep\': comprehensive search' }),
    'context-max-characters': defineOption(z.coerce.number().optional(), { description: 'Maximum characters for context string optimized for LLMs (default: 10000)' }),
    'raw': defineOption(z.string().optional(), { description: 'Raw JSON arguments' }),
    'timeout': defineOption(z.coerce.number().optional(), { description: 'Timeout in ms', short: 't' }),
    'output': defineOption(z.enum(['text', 'json', 'raw']).optional(), { description: 'Output format', short: 'o' }),
  },
  handler: async ({ flags }) => {
...
const ExaCompanyResearchExaCommand = defineCommand({
  ...
  options: {
    'company-name': defineOption(z.string(), { description: 'Name of the company to research' }),
    'num-results': defineOption(z.coerce.number().optional(), { description: 'Number of search results to return (default: 5)' }),
    'raw': defineOption(z.string().optional(), { description: 'Raw JSON arguments' }),
    'timeout': defineOption(z.coerce.number().optional(), { description: 'Timeout in ms', short: 't' }),
    'output': defineOption(z.enum(['text', 'json', 'raw']).optional(), { description: 'Output format', short: 'o' }),
  },
...
const ExaGetCodeContextExaCommand = defineCommand({
  ...
  options: {
    'query': defineOption(z.string(), { description: 'Search query to find relevant context for APIs, Libraries, and SDKs. For example, \'React useState hook examples\', \'Python pandas dataframe filtering\', \'Express.js middleware\', \'Next js partial prerendering configuration\'' }),
    'tokens-num': defineOption(z.coerce.number().min(1000).max(50000).default(5000), { description: 'Number of tokens to return (1000-50000). Default is 5000 tokens. Adjust this value based on how much context you need - use lower values for focused queries and higher values for comprehensive documentation.' }),
    'raw': defineOption(z.string().optional(), { description: 'Raw JSON arguments' }),
    'timeout': defineOption(z.coerce.number().optional(), { description: 'Timeout in ms', short: 't' }),
    'output': defineOption(z.enum(['text', 'json', 'raw']).optional(), { description: 'Output format', short: 'o' }),
  },
