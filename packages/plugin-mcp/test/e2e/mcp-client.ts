 ...
 */
export async function fetchToolsFromStdio(config: StdioServerConfig): Promise<MCPTool[]> {
  ...
  const client = new Client({ name: "bunli-mcp-test", version: "1.0.0" }, { capabilities: {} });
 ...
 */
export async function fetchToolsFromHttp(config: HttpServerConfig): Promise<MCPTool[]> {
  const url = new URL(config.url);
  const transportType = config.transport ?? "streamable";

  const transport =
    transportType === "sse" ? new SSEClientTransport(url) : new StreamableHTTPClientTransport(url);
