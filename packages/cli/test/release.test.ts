...
describe("resolvePlatformsStrict", () => {
  test("throws on unsupported targets", () => {
    expect(() => resolvePlatformsStrict(["linux-x64", "bogus-target"])).toThrow(
  ...
  test("throws when targets resolve to zero platforms", () => {
    ...
    if (
      ["darwin-arm64", "darwin-x64", "linux-arm64", "linux-x64", "windows-x64"].includes(
        nativeTarget,
      )
    ) {
      return;
...
describe("buildPlatformPackageName", () => {
  test("default {{name}}-{{platform}} format", () => {
    expect(buildPlatformPackageName("{{name}}-{{platform}}", "my-cli", "darwin-arm64")).toBe(
      "my-cli-darwin-arm64",
    );
    expect(buildPlatformPackageName("{{name}}-{{platform}}", "tool", "windows-x64")).toBe(
      "tool-windows-x64",
    );
  });

  test("inverted {{platform}}-{{name}} format", () => {
    expect(buildPlatformPackageName("{{platform}}-{{name}}", "my-cli", "linux-x64")).toBe(
      "linux-x64-my-cli",
    );
  });

  test("scoped npm package name", () => {
    expect(buildPlatformPackageName("{{name}}-{{platform}}", "@scope/cli", "darwin-arm64")).toBe(
      "@scope/cli-darwin-arm64",
    );
  });

  test("custom format with prefix", () => {
    expect(buildPlatformPackageName("@my-org/{{name}}-{{platform}}", "tool", "linux-arm64")).toBe(
      "@my-org/tool-linux-arm64",
    );
  });
...
describe("buildShimPlatformMap", () => {
  ...
  test("handles all 5 platforms, remapping windows", () => {
    ...
    const platforms = ["darwin-arm64", "darwin-x64", "linux-arm64", "linux-x64", "windows-x64"];
    for (const p of platforms)
      published.push({ platform: p, packageName: `cli-${p}`, version: "1.0.0" });
...
function makePrompt(selectValue: string, textValue = ""): PromptApi {
  return Object.assign((_msg: string) => Promise.resolve(textValue), {
    confirm: () => Promise.resolve(true),
    select: <T>(_msg: string, opts: { options: Array<{ value: T }> }) =>
      Promise.resolve(
        opts.options.find((o) => String(o.value) === selectValue)?.value ?? opts.options[0].value,
      ),
...
): Promise<{ tmpDir: string; originalCwd: string }> {
...
describe("release command - dry run, default config", () => {
  ...
  test("--version=1.5.0-rc.1 passes custom version through", async () => {
    const result = await testCommand(releaseCommand, {
      flags: { version: "1.5.0-rc.1", dry: true, npm: false },
    });
  ...
  test("--npm=false skips npm publish step", async () => {
    const result = await testCommand(releaseCommand, {
      flags: { version: "patch", dry: true, npm: false },
    });
  ...
  test("typed --resume=false skips dry-run resume-state notice", async () => {
    ...
    const result = await testCommand(releaseCommand, {
      flags: { version: "patch", dry: true, resume: false },
    });
...
describe("release command - config npm:false", () => {
  ...
  test("--npm flag overrides config.release.npm", async () => {
    const result = await testCommand(releaseCommand, {
      flags: { version: "patch", dry: true, npm: true },
    });
    expect(result.stdout).toContain("✅ Publishing to npm");
    expect(result.exitCode).toBe(0);
  });
...
describe("release command - config github:true", () => {
  ...
  beforeEach(async () => {
    ({ tmpDir, originalCwd } = await createFixture("export default { release: { github: true } }"));
  });
  ...
  test("--github=false flag overrides config.release.github", async () => {
    const result = await testCommand(releaseCommand, {
      flags: { version: "patch", dry: true, github: false },
    });
    expect(result.stdout).not.toContain("Creating GitHub release");
    expect(result.exitCode).toBe(0);
  });
...
describe("release command - unsupported workflows", () => {
  ...
  test("--all fails with clear not-implemented error", async () => {
    ...
    try {
      const result = await testCommand(releaseCommand, {
        flags: { version: "patch", dry: true, all: true },
      });
