...
function parseReleaseState(value: unknown): ReleaseState {
  ...
  if ('schemaVersion' in state) {
    throw new Error('Legacy release state format is no longer supported. Delete .bunli/release-state.json and rerun release.')
  }
  ...
  if (!Array.isArray(state.completedSteps) || state.completedSteps.some(step => !isReleaseStepId(step))) {
