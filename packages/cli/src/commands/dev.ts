...
import {
  getInstallRequiredMessage,
  hasInstalledProjectDependencies,
} from "../utils/install-check.js";
...
export default defineCommand({
  ...
  options: {
    ...
    generate: option(z.boolean().default(true), {
      description: "Enable codegen",
      argumentKind: "flag",
    }),
    clearScreen: option(z.boolean().default(true), {
    ...
    inspect: option(z.boolean().default(false), {
      short: "i",
...
async function runDev(
  ...
  const generateTypes = async (): Promise<Result<void, DevCommandErrorType>> => {
    ...
    const configuredEntryFile = Array.isArray(configuredEntry)
      ? configuredEntry[0]
      : configuredEntry;
    ...
    if (!resolvedEntry) {
      return failDev(
        "No entry file found for code generation. Set commands.entry or pass --entry.",
      );
    }
    ...
    if (Result.isError(generationResult)) {
      return failDev(
        `Failed to generate types: ${generationResult.error.message}`,
  ...
  if (typedFlags.watch ?? config.dev?.watch ?? true) {
    ...
    if (typedFlags.generate && watchDirectory && existsSync(watchDirectory)) {
      ...
      console.log(
        colors.yellow(`Skipping command watcher: directory does not exist (${watchDirectory})`),
      );
    } else if (typedFlags.generate && !watchDirectory) {
      console.log(
        colors.yellow(
          "Skipping command watcher: set commands.directory to avoid watching the project root",
        ),
      );
    }
  }
