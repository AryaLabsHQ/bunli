...
function parseTargetPlatformPair(target: string): { os: string; cpu: string } | null {
  const normalizedTarget = target.startsWith("bun-") ? target.slice(4) : target;
...
export default defineCommand({
  ...
  options: {
    ...
    minify: option(z.boolean().optional(), {
      short: "m",
      description: "Minify output",
      argumentKind: "flag",
    }),
    sourcemap: option(z.boolean().optional(), {
      short: "s",
    ...
    if (isCompiling) {
      ...
      if (isPackageResolvable("@opentui/core")) {
        const missingOpenTuiTargets = platformTargets
          ...
          .filter((entry): entry is { platform: string; packageName: string } =>
      ...
      for (const platform of platformTargets) {
        ...
        const outfile =
          typedFlags.outfile || path.join(targetDir, isWindows ? `${baseName}.exe` : baseName);
        ...
        if (typedFlags.sourcemap ?? config.build?.sourcemap ?? false)
