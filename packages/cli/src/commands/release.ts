...
export function resolvePlatformsStrict(targets: string[]): string[] {
  ...
  const unsupportedTargets = targets.filter(
    (t) => t !== "all" && t !== "native" && !(t in PLATFORM_META),
  );
  if (unsupportedTargets.length > 0) {
    throw new Error(
      `Unsupported build.targets for binary release: ${unsupportedTargets.join(", ")}`,
    );
  }
...
export function buildPlatformPackageName(
  format: string,
  cliName: string,
  platform: string,
): string {
...
export function buildShimPlatformMap(
  published: PublishedPlatformPackage[],
): Record<string, string> {
  const platformMap: Record<string, string> = {};
  for (const item of published) {
    const key = item.platform.startsWith("windows")
      ? item.platform.replace("windows", "win32")
      : item.platform;
...
export default defineCommand({
  ...
  options: {
    ...
    npm: option(z.boolean().optional(), { description: "Publish to npm", argumentKind: "flag" }),
    github: option(z.boolean().optional(), {
  ...
  handler: async ({ flags, prompt, spinner, colors, runtime, terminal }) => {
    ...
    if (unsupportedFlags.length > 0) {
      console.error(
        colors.red(
          `Unsupported flags: ${unsupportedFlags.join(", ")}. Use --npm=false, --github=false, or --resume=false.`,
        ),
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (status.trim() && !flags.dry) {
        console.error(
          colors.red("Working directory is not clean. Please commit or stash changes first."),
        );
        process.exit(1);
      }
    } catch {
...
async function releaseSingle(
  ...
  if (resumeState) {
    if (flags.version !== undefined) {
      console.error(
        colors.red(
          "Cannot use --version when resuming. Use --resume=false to start a fresh release.",
        ),
      );
      process.exit(1);
    }
    if (flags.tag !== undefined) {
      console.error(
        colors.red("Cannot use --tag when resuming. Use --resume=false to start a fresh release."),
      );
      process.exit(1);
    }
    if (flags.npm !== undefined && flags.npm !== resumeState.options.publishNpm) {
      console.error(
        colors.red(
          "Cannot change --npm while resuming. Use --resume=false to start a fresh release.",
        ),
      );
      process.exit(1);
    }
    if (flags.github !== undefined && flags.github !== resumeState.options.publishGitHub) {
      console.error(
        colors.red(
          "Cannot change --github while resuming. Use --resume=false to start a fresh release.",
        ),
      );
      process.exit(1);
    }
  ...
  const publishNpm = resumeState
    ? resumeState.options.publishNpm
    : (flags.npm ?? config.release.npm);
  const publishGitHub = resumeState
    ? resumeState.options.publishGitHub
    : (flags.github ?? config.release.github);
  ...
  if (!resumeState && newVersion === currentVersion) {
    console.error(
      colors.red(`Version unchanged: ${newVersion}. Please choose a different version.`),
    );
    process.exit(1);
  }
  ...
  if (binaryConfig && publishNpm) {
    if (config.build.compress) {
      console.error(
        colors.red(
          "Binary release is incompatible with build.compress: true - raw binaries are removed after compression.",
        ),
      );
      process.exit(1);
    }
  ...
  let publishedPlatforms: PublishedPlatformPackage[] = resumeState
    ? getPublishedPlatformsFromState(resumeState)
    : [];

  console.log(
    colors.bold(`${resumeState ? "Resuming release for" : "Releasing"} ${pkg.name || "CLI"}`),
  );
  if (resumeState) {
      ...
      ? [
          ...
          },
          {
            ...
            probe: async () => {
              ...
              return mainPackageBinaryStateMatches(
                publishedPlatforms,
                binaryConfig!.shimPath,
                cliBinName,
              );
            },
            cmd: async () => {
              ...
              if (publishedPlatforms.length === 0) {
                throw new Error(
                  "No platform packages available to update main package for binary mode",
                );
              }
              await updateMainPackageForBinary(
                publishedPlatforms,
                binaryConfig!.shimPath,
                cliBinName,
              );
  ...
  if (publishGitHub) {
    console.log(
      colors.dim(`GitHub: https://github.com/${await getGitHubRepo()}/releases/tag/${tag}`),
    );
  }
  if (publishNpm) {
...
export async function determineVersion(
  ...
  const choice = await prompt.select<"patch" | "minor" | "major" | "custom">(
    "Select version bump:",
    {
      options: [
...
async function publishPlatformPackages(opts: {
  ...
  const publishedMap = new Map<string, PublishedPlatformPackage>(
    known.map((item) => [item.platform, item]),
  );
  ...
  for (const platform of context.platforms) {
    ...
    if (
      alreadyKnown &&
      alreadyKnown.packageName === pkgName &&
      alreadyKnown.version === newVersion
    ) {
      continue;
    }
  ...
  const ordered = context.platforms
    .map((platform) => publishedMap.get(platform))
    .filter(Boolean) as PublishedPlatformPackage[];
...
async function updateMainPackageForBinary(
  ...
  const optionalDeps = Object.fromEntries(
    published.map((item) => [item.packageName, item.version]),
  );
...
function generateShimContent(published: PublishedPlatformPackage[], cliBinName: string): string {
...
async function createGitTag(opts: {
  tag: string;
  ...
  if (staged.trim()) {
    const commitMessage = conventionalCommits ? `chore(release): ${tag}` : `chore: release ${tag}`;
...
function validateResumeStateOrExit(
  ...
  if (state.packageName !== pkg.name) {
    console.error(
      colors.red(`Resume state package mismatch: expected ${state.packageName}, found ${pkg.name}`),
    );
    process.exit(1);
  }
  ...
  if (binaryContext) {
    ...
    if ((state.options.shimPath ?? "") !== (binaryConfig?.shimPath ?? "")) {
      console.error(
        colors.red("Resume state shimPath does not match current release.binary.shimPath."),
      );
      process.exit(1);
    }
  }
...
async function getAllPublishedPlatformPackages(opts: {
  ...
  const byPlatform = new Map<string, PublishedPlatformPackage>(
    known.map((item) => [item.platform, item]),
  );
