...
function collectNestedExpectedChildren(
  ...
  for (const entry of entries) {
    ...
    if (
      !rootPath ||
      !Array.isArray(entry.metadata?.commands) ||
      entry.metadata.commands.length === 0
    ) {
      continue;
    }
...
export default defineCommand({
  ...
  async handler({ flags, colors }) {
    ...
    if (!(await generatedFile.exists())) {
      throw new Error(
        `Missing generated metadata file: ${generatedPath}. Run "bunli generate" first.`,
      );
    }
    ...
    for (const entry of entries) {
      const runtimeName = typeof entry.name === "string" ? entry.name.trim() : "";
      const metadataName =
        typeof entry.metadata?.name === "string" ? entry.metadata.name.trim() : "";
      const effectiveName = metadataName || runtimeName;
    ...
    const hasNestedPath = Array.from(normalizedNames).some(
      (name) => name.includes("/") || name.includes(" "),
    );
    if (hasNestedPath && !Array.from(normalizedNames).some((name) => name.includes("/"))) {
      warnings.push(
        'Nested command names detected without "/" separators. Ensure completion graph normalization is correct.',
      );
    }
    ...
    console.log(
      colors.green(`✓ Loaded ${entries.length} generated command entries from ${generatedPath}`),
    );
