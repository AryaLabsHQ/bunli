...
export default defineCommand({
  ...
  options: {
    ...
    files: option(z.boolean().optional().default(true), {
      description: "Show files",
      argumentKind: "flag",
    }),
    directories: option(z.boolean().optional().default(true), {
      description: "Show directories",
      argumentKind: "flag",
    }),
    hidden: option(z.boolean().optional().default(false), {
      description: "Show hidden files",
      argumentKind: "flag",
    }),
    extensions: option(z.string().optional(), {
  ...
  async handler({ flags, prompt }) {
    ...
    const result = selected.endsWith("/") ? join(dir, selected.slice(0, -1)) : join(dir, selected);
    writeStdout(result);
  },
});
